...
func main() {
	...
	// Returns Ortho, Diag
	TEXT("queenAttacks", NOSPLIT, "func(occupied uint64, location uint64, rank, file, diag, antidiag uint64) uint64")
	occ := Load(Param("occupied"), GP64())
	pos := Load(Param("location"), GP64())
	rank := Load(Param("rank"), GP64())
	file := Load(Param("file"), GP64())
	diag := Load(Param("diag"), GP64())
	antidiag := Load(Param("antidiag"), GP64())
	...
	MOVQ(out, outDiag)
	ORQ(outOrtho, outDiag)
	Store(outDiag, ReturnIndex(0))
	RET()

	// Bishop Only
	doBishopAttacks(bytes)
	Generate()
}

func doBishopAttacks(bytes Mem) {
	TEXT("bishopRookAttacks", NOSPLIT, "func(occupied uint64, location uint64, rankOrDiag, fileOrAntiDiag uint64) uint64")
	occ := Load(Param("occupied"), GP64())
	pos := Load(Param("location"), GP64())
	diag := Load(Param("rankOrDiag"), GP64())
	antidiag := Load(Param("fileOrAntiDiag"), GP64())
	bytesPtr := Mem{Base: GP64()}
	LEAQ(bytes, bytesPtr.Base)
	shuf := XMM()
	rev := [3]reg.VecVirtual{XMM(), XMM(), XMM()}
	...
	Comment("Shift pos")
	PSLLQ(U8(1), posX)
	Comment("Reverse data")
	reverse64(data, rev, shuf)
	Comment("Subtract second half")
	VPSUBQ(posX, data, data)
	Comment("Unreverse data")
	reverse64(data, rev, shuf)
	Comment("Finish")
	PXOR(nonrev, data)
	PAND(mask, data)
	Comment("Extract")
	outa, outb := GP64(), GP64()
	PEXTRQ(U8(1), data, outa)
	MOVQ(data, outb)
	ORQ(outa, outb)
	Store(outb, ReturnIndex(0))
	RET()
